...
fn phong_fs(in: VertexOutput) -> vec4<f32> {
    ...
    for (var i: i32 = 0; i < min(lights.light_count, MAX_LIGHT_COUNT); i = i + 1) {
        ...
        let diffuse = object_color * light.color * light.color.a * NdotL;

        // Simple Blinn-Phong specular
        let halfway_dir = normalize(light_dir + view_dir);
        let specular = pow(max(dot(mapped_normal, halfway_dir), 0.0), 32.0);

        let atenuation = 1.0 / max(distance * distance, 0.001);
...
fn cell_fs(in: VertexOutput, light_index: i32) -> vec4<f32> {
    ...
    for (var i: i32 = 0; i < min(lights.light_count, MAX_LIGHT_COUNT); i = i + 1) {
        ...
        // Simple Lambertian diffuse
        let NdotL = max(dot(mapped_normal, light_dir), 0.0);
        let toon_diffuse = cel_shade(NdotL, TOON_LEVELS);

        // Simple Blinn-Phong specular
        let halfway_dir = normalize(light_dir + view_dir);
        let specular = pow(max(dot(mapped_normal, halfway_dir), 0.0), 32.0);

        let atenuation = 1.0 / max(distance, 0.001);

        total_light += (object_color + toon_diffuse) * atenuation;
    }
    return vec4<f32>(total_light.rgb, object_color.a);
}
