...
/// which `ContentAssetRoot::default_for_platform` discovers from the executable
/// name. Every example in this workspace builds into the same shared Cargo
/// `target/` directory, so each gets its own
